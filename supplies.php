<?php
	include 'db.php';
	$sql = "SELECT kunde_lieferant.Name AS Lieferantenname, bestellung.Bestell_ID, ware.Name, bestellung.Menge, bestellung.Menge * ware.Verkaufspreis AS Warenwert
...
ORDER BY kunde_lieferant.Kennung, bestellung.Bestell_ID DESC";
	...
	echo "Lieferung von ".$row["Lieferantenname"].".";
	$result = mysqli_query($connection,$sql) or die ("Fehlgeschlagen! SQL-Error:".mysqli_error($connection));
...
while ($row = mysqli_fetch_assoc($result)) {
  $gesamtwert += $row["Warenwert"];
	...
	echo "Der Lieferant hat Ware im Wert von ".$gesamtwert."€ geliefert.";
?>
